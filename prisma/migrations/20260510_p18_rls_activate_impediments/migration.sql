-- Wave P18 hardening — Activación incremental RLS restrictivas (Impediment).
--
-- Aplica la política `Impediment_member_only` y DESACTIVA la
-- open-policy heredada. Impediment NO tiene projectId directo · se
-- resuelve la pertenencia vía subquery a Sprint (sprintId → projectId).
--
-- Pre-requisito: las server actions de impediments.ts deben pasar por
...
-- ── Impediment ──
-- Variante con subquery: Impediment no tiene projectId. Resolvemos
...
CREATE POLICY "Impediment_member_only" ON "Impediment"
  ...
  USING (
    EXISTS (
      ...
      WHERE s."id" = "Impediment"."sprintId"
