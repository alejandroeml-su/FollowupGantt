...
function softwareProjectWBS(projectName: string, teamSize: number): WBSGenerated {
  ...
  return {
    ...
    risks: [
      {
        description: 'Cambio de alcance durante implementación',
        mitigation: 'Definir gates de change-control y backlog priorizado por valor',
      },
      {
        description: 'Disponibilidad limitada de stakeholders en UAT',
        mitigation: 'Calendarizar sesiones de UAT con 2 semanas de anticipación',
      },
      {
        description: 'Bloqueo por integración de terceros',
        mitigation: 'Mockear servicios externos durante el desarrollo y validar APIs en spike inicial',
      },
    ],
  }
}
...
function marketingCampaignWBS(projectName: string, teamSize: number): WBSGenerated {
  ...
  return {
    ...
    risks: [
      {
        description: 'Retraso en aprobaciones de marca o legales',
        mitigation: 'Subir piezas a revisión por lotes con SLA de 3 días hábiles',
      },
      {
        description: 'Variación del tipo de cambio o costos de pauta',
        mitigation: 'Reservar 15% de buffer presupuestario y monitorear semanalmente',
      },
      {
        description: 'Bajo engagement inicial post-lanzamiento',
...
function infrastructureDeployWBS(projectName: string, teamSize: number): WBSGenerated {
  ...
  return {
    ...
    risks: [
      {
        description: 'Pérdida de datos durante migración',
        mitigation: 'Snapshot full + verificación checksum pre/post migración',
      },
      {
        description: 'Ventana de cutover excede el tiempo planeado',
        mitigation: 'Ensayo de cutover en ambiente staging con cronómetro',
      },
      {
        description: 'Costos cloud por encima del presupuesto',
        mitigation: 'Activar budgets/alerts en el día 1 y revisar tagging por proyecto',
      },
    ],
  }
}
