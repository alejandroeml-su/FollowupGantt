...
export interface ScenarioTaskInput {
  ...
  endDate: string | null
  /** Día final del proyecto (último endDate de cualquier task). */
}
...
export interface AllocationUserInput {
  ...
  projects: Array<{ projectId: string; projectName: string; spi: number | null; taskCount: number }>
}
...
export function simulateDelay(input: {
  ...
  while (queue.length > 0 && safetyCounter++ < 5000) {
    ...
    for (const dep of succs) {
      ...
      queue.push({ taskId: sucTask.id, newEndMs: newSucEndMs, depth: node.depth + 1 })
    }
  }

  const affected = Array.from(impacts.values()).sort((a, b) => b.deltaDays - a.deltaDays)
  ...
  const originalProjectEndMs = originalEnds.length > 0 ? Math.max(...originalEnds) : null
  ...
  return {
    ...
    newProjectEndDate: newProjectEndMs != null ? new Date(newProjectEndMs).toISOString() : null,
    originalProjectEndDate:
      originalProjectEndMs != null ? new Date(originalProjectEndMs).toISOString() : null,
  }
}
  ...
  for (const u of overcommitted) {
    const avgSpi =
      u.projects.length > 0
        ? u.projects
            .map((p) => p.spi ?? 1)
            .reduce((s, n) => s + n, 0) / u.projects.length
        : null
    const aheadProjects = u.projects.filter((p) => p.spi != null && p.spi > 1.05)
    const behindProjects = u.projects.filter((p) => p.spi != null && p.spi < 0.95)
    const severity: BalanceSuggestion['severity'] =
      u.totalDailyHours > 12 ? 'HIGH' : u.totalDailyHours > 10 ? 'MEDIUM' : 'LOW'
